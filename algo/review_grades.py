...
def review_grade(truth,score,empirical_scores,skip_loss,loss=quadratic_loss):
    ...
    if sl != skip_loss:
        logger.info('review_grade(...): skip_loss set to ' + str(sl))
    skip_loss = sl
    ...
    if avg_loss <= 0.0:
        logger.info('review_grade(...): average_loss is zero')
...
def review_grades(reviews, truths, skip_loss,loss=quadratic_loss):
    ...
    if nogrades:
         logger.info('grade_reviews(...): There are peers with no\
                     review grades: ' + str(nogrades))
